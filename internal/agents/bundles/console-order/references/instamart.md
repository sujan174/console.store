...
5. `place_order {confirmation_id}` — returns the placed `order` directly
   (COD — no UPI leg, unlike food). Never call it uninvited, never retry it;
   on error check `list_active_orders` first.
...
- COD only; typical delivery 10–20 min; no cancellation (customer care
  080-67466729).
